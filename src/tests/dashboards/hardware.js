...
module.exports = {
    ...
    'Chart titles and order are correct': browser => {
        flagForUpdate(browser, '#dashboard-sections .mg-chart-title', 'metrics in the hardware dashboard', 12);

        browser.expect.element('#dashboard-sections .dashboard-section:nth-child(1) #graphics-metric-overview-1 .mg-chart-title').text.to.be.equal('GPU Model');
        browser.expect.element('#dashboard-sections .dashboard-section:nth-child(1) #graphics-metric-overview-2 .mg-chart-title').text.to.be.equal('GPU Vendor');
        browser.expect.element('#dashboard-sections .dashboard-section:nth-child(1) #graphics-metric-overview-3 .mg-chart-title').text.to.be.equal('Display Resolution');
        browser.expect.element('#dashboard-sections .dashboard-section:nth-child(2) #processor-metric-overview-1 .mg-chart-title').text.to.be.equal('CPU Vendor');
        browser.expect.element('#dashboard-sections .dashboard-section:nth-child(2) #processor-metric-overview-2 .mg-chart-title').text.to.be.equal('CPU Cores');
        browser.expect.element('#dashboard-sections .dashboard-section:nth-child(2) #processor-metric-overview-3 .mg-chart-title').text.to.be.equal('CPU Speeds');
        browser.expect.element('#dashboard-sections .dashboard-section:nth-child(2) #processor-metric-overview-4 .mg-chart-title').text.to.be.equal('Memory');
        browser.expect.element('#dashboard-sections .dashboard-section:nth-child(3) #operatingsystem-metric-overview-1 .mg-chart-title').text.to.be.equal('Operating System');
        browser.expect.element('#dashboard-sections .dashboard-section:nth-child(3) #operatingsystem-metric-overview-2 .mg-chart-title').text.to.be.equal('Browsers by Architecture');
        browser.expect.element('#dashboard-sections .dashboard-section:nth-child(3) #operatingsystem-metric-overview-3 .mg-chart-title').text.to.be.equal('Operating Systems by Architecture');
        browser.expect.element('#dashboard-sections .dashboard-section:nth-child(4) #plugins-metric-overview-1 .mg-chart-title').text.to.be.equal('Flash');
        browser.expect.element('#dashboard-sections .dashboard-section:nth-child(4) #plugins-metric-overview-2 .mg-chart-title').text.to.be.equal('Unity');
    },

    'Charts render': browser => {
        flagForUpdate(browser, '#dashboard-sections .metric-overview', 'metrics in the hardware dashboard', 12);
